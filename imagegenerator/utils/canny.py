...
def Gaussian_Filter(kernel_size=GAUS_KERNEL, sigma=GAUS_SIGMA):  # Default: Filter_shape = [5,5]
    ...
    for i in range(kernel_size):
        ...
        for j in range(kernel_size):
            Hij = np.exp(-((i + 1 - (k + 1)) ** 2.0 + (j + 1 - (k + 1)) ** 2) / (2.0 * sigma_2)) / (2 * np.pi * sigma_2)
...
def TF_Canny(img_tensor, minRate=0.050, maxRate=0.40,
...
if __name__ == '__main__':  # Test the above code
    ...
    # img[3, 0:5] = 100
    # img[4, 0:4] = 100
    img = np.zeros(shape=(2, 128, 128, 1))

    img_ = cv2.imread('13.jpg')
    img_ = cv2.cvtColor(img_, cv2.COLOR_BGR2GRAY)
    img_ = cv2.resize(img_, (128, 128))

    img__ = cv2.imread('11.jpg')
    img__ = cv2.cvtColor(img__, cv2.COLOR_BGR2GRAY)
    img__ = cv2.resize(img__, (128, 128))
    ...
    input_img = tf.placeholder(tf.float32, [2, 128, 128, 1])
    # x_tensor = tf.expand_dims(tf.expand_dims(input_img, axis=0), -1)
    edges_tensor = TF_Canny(input_img)  # , return_raw_edges=True)

    with tf.Session() as sess:
        with tf.device('gpu/:0'):
            edges_tf = sess.run(edges_tensor, feed_dict={input_img: img})

    # plt.subplot(131), plt.imshow(img, cmap='gray')
    # plt.title('Original Image'), plt.xticks([]), plt.yticks([])
    plt.subplot(132), plt.imshow(edges_tf[0, :, :], cmap='gray')
    plt.title('Edges-OpenCV.Canny'), plt.xticks([]), plt.yticks([])
    plt.subplot(133), plt.imshow(edges_tf[1, :, :], cmap='gray')
    plt.title('Edges-OpenCV.Canny'), plt.xticks([]), plt.yticks([])
